...
async function queryRegridParcel(lat, lon) {
  ...
  const data = await response.json();
  console.log('Regrid response:', JSON.stringify(data).substring(0, 1000));
...
<body class="bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen p-6">
    <div class="max-w-6xl mx-auto">
        <div class="bg-white rounded-lg shadow-lg p-8 mb-6">
            <h1 class="text-3xl font-bold text-gray-800 mb-4">PA Property Lookup - Horst Signs</h1>
            <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                <p class="text-sm text-green-800"><strong>✓ Connected</strong> - Regrid API Integrated</p>
            </div>
            <div class="mb-6">
            ...
            <div id="resultsContainer" class="hidden space-y-6"><div class="bg-blue-50 rounded-lg p-6 border border-blue-200"><h2 class="text-xl font-bold text-gray-800 mb-4">Property Details - <span id="countyBadge"></span></h2><div id="propertyGrid" class="grid grid-cols-2 gap-4"></div></div><div class="flex gap-4"><button onclick="window.print()" class="flex-1 px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700">Print Report</button><button onclick="clearResults()" class="px-6 py-3 bg-gray-400 text-white rounded-lg hover:bg-gray-500">Clear</button></div></div>
        </div>
        <div class="bg-white rounded-lg shadow-lg p-6"><h3 class="text-lg font-bold text-gray-800 mb-3">Example Addresses</h3><div class="space-y-2 text-sm"><p class="cursor-pointer hover:text-indigo-600" onclick="useAddress('50 N Duke St, Lancaster, PA 17602')">• 50 N Duke St, Lancaster, PA 17602</p><p class="cursor-pointer hover:text-indigo-600" onclick="useAddress('1 Park City Center, Lancaster, PA 17601')">• 1 Park City Center, Lancaster, PA 17601</p></div></div>
    </div>
    <script>
